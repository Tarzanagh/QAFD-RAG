...
from text chunks for knowledge graph construction.
"""
...
async def _handle_entity_relation_summary(
    ...
    """Summarize entity or relationship descriptions using LLM.

    If the description exceeds token limits, uses LLM to create a summary.

    Args:
        entity_or_relation_name: Name of the entity or relationship
        description: Full description text
        global_config: Configuration with LLM settings

    Returns:
        Original description if under limit, otherwise summarized text
    """
...
async def _handle_single_entity_extraction(
    ...
    """Parse a single entity record from LLM output.

    Args:
        record_attributes: List of record attributes from LLM
        chunk_key: Source chunk identifier

    Returns:
        Entity dictionary or None if invalid
    """
...
async def _handle_single_relationship_extraction(
    ...
    """Parse a single relationship record from LLM output.
...
async def _merge_nodes_then_upsert(
    ...
    """Merge duplicate entity nodes and upsert to knowledge graph.
...
async def _merge_edges_then_upsert(
    ...
    """Merge duplicate relationship edges and upsert to knowledge graph.
...
async def extract_entities(
    ...
    This function processes text chunks in parallel, using an LLM to extract
    entities and relationships, then merges duplicates and stores them in
    the knowledge graph and vector databases.
    ...
    Args:
        ...
        global_config: Configuration dictionary with LLM settings

    Returns:
        Updated knowledge graph instance, or None if extraction failed
    """
    use_llm_func: callable = global_config["llm_model_func"]
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # Use a safer approach: first format the prompt template with a placeholder for input_text
